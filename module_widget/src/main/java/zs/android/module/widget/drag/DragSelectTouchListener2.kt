 ...
 * @mark 自定义类描述
 */
class DragSelectTouchListener2 : RecyclerView.OnItemTouchListener {
    private val TAG = "DSTL"
    ...
    private var mSelectListener2: OnDragSelectListener? = null
     ...
     */
    fun withSelectListener(selectListener: OnDragSelectListener?): DragSelectTouchListener2 {
        mSelectListener2 = selectListener
        return this
    }
     ...
     */
    fun withMaxScrollDistance(distance: Int): DragSelectTouchListener2 {
     ...
     */
    fun withTouchRegion(size: Int): DragSelectTouchListener2 {
     ...
     */
    fun withTopOffset(distance: Int): DragSelectTouchListener2 {
     ...
     * @param distance offset
     */
    fun withBottomOffset(distance: Int): DragSelectTouchListener2 {
     ...
     */
    fun withScrollAboveTopRegion(enabled: Boolean): DragSelectTouchListener2 {
     ...
     */
    fun withScrollBelowTopRegion(enabled: Boolean): DragSelectTouchListener2 {
        mScrollBelowTopRegion = enabled
        return this
    }

    fun withDebug(enabled: Boolean): DragSelectTouchListener2 {
        mDebug = enabled
        return this
    }
    ...
    fun startDragSelection(position: Int) {
        ...
        mLastStart = position
        mLastEnd = position
        if (mSelectListener2 != null && mSelectListener2 is OnAdvancedDragSelectListener) {
            (mSelectListener2 as OnAdvancedDragSelectListener).onSelectionStarted(position)
        }
    }
    ...
    override fun onInterceptTouchEvent(rv: RecyclerView, e: MotionEvent): Boolean {
        if (!mIsActive || rv.adapter!!.itemCount == 0) {
            return false
        }
    ...
    private fun initScroller(context: Context) {
        if (mScroller == null) {
            ...
            mScroller = if (interpolator != null) {
                OverScroller(context, LinearInterpolator())
            } else {
    ...
    private fun processAutoScroll(event: MotionEvent) {
        ...
        if (y >= mTopBoundFrom && y <= mTopBoundTo) {
            ...
            if (mDebug) {
                Log.d(
                    TAG,
                    "SCROLL - mScrollSpeedFactor=$mScrollSpeedFactor | mScrollDistance=$mScrollDistance"
                )
            }
            if (!mInTopSpot) {
                mInTopSpot = true
    ...
    private fun notifySelectRangeChange() {
        ...
        if (mLastStart == RecyclerView.NO_POSITION || mLastEnd == RecyclerView.NO_POSITION) {
            if (newEnd - newStart == 1) {
                mSelectListener2!!.onSelectChange(newStart, newStart, true)
            } else {
                mSelectListener2!!.onSelectChange(newStart, newEnd, true)
            }
        } else {
            if (newStart > mLastStart) {
                mSelectListener2!!.onSelectChange(mLastStart, newStart - 1, false)
            } else if (newStart < mLastStart) {
                mSelectListener2!!.onSelectChange(newStart, mLastStart - 1, true)
            }
            if (newEnd > mLastEnd) {
                mSelectListener2!!.onSelectChange(mLastEnd + 1, newEnd, true)
            } else if (newEnd < mLastEnd) {
                mSelectListener2!!.onSelectChange(newEnd + 1, mLastEnd, false)
            }
        }
        mLastStart = newStart
        mLastEnd = newEnd
    ...
    private fun reset() {
        setIsActive(false)
        if (mSelectListener2 != null && mSelectListener2 is OnAdvancedDragSelectListener) {
            (mSelectListener2 as OnAdvancedDragSelectListener).onSelectionFinished(mEnd)
        }
