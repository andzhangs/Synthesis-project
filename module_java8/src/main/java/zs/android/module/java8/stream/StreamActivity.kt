package zs.android.module.java8.stream
...
class StreamActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mList.stream().apply {
            this.filter { it.sex == "男" }.forEach(System.out::println)
            this.sorted().filter { it.name.startsWith("T") }.forEach(System.out::println)
            this.anyMatch { it.name.startsWith("T") }
            this.allMatch { it.name.startsWith("F") }
            this.noneMatch { it.name.startsWith("F") }
            this.filter { it.sex.toInt()>18 }.count()
    ...
    private fun stream() {
        sb.append("filter操作符：\n")
        mList.stream().filter { "男" == it.sex }.collect(Collectors.toList()).forEach {
        ...
        mList.stream().map { it.name }.collect(Collectors.toList()).forEach {
            sb.append("\t\t").append(it).append("\n")
        }.apply {
            tv.text = sb.toString()
        }
    ...
    private fun collectTest() {
        //toList
        val list = mList.stream().map(UserModel::name).collect(Collectors.toList())
        //toSet
...
//        val treeSet=mList.stream().collect(Collectors.toCollection(TreeSet::new))
    ...
    private fun optionalTest() {
        ...
        val isNull =
            Optional.ofNullable(model).map(EarthModel::model)
                .map { return@map SubModel::name }.isPresent
    ...
    override fun onResume() {
        ...
        setResult(RESULT_OK, Intent().apply {
            putExtra(MainActivity.RESULT_VALUE, this@StreamActivity::class.java.simpleName)
        })
    }
}